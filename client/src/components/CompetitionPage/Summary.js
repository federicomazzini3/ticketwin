import { Button, Grid, Stack, Dialog, DialogTitle, DialogContent, DialogContentText, DialogActions } from '@mui/material'
    ...
    <>
      <Grid item xs={12} sx={{ mt: 10, mb: -3, width: "100%", alignItems: 'center', justifyItems: 'center' }}>
        ...
        <Stack direction="row" spacing={2} justifyContent="center" >
          <Button variant='contained' size='large' startIcon={<ShoppingCartIcon />} component={Link} onClick={() => handleClick()} >
            Purchase now ticket {competitionTicketsOnly().tickets.map(t => `${t.number}`).join(', ')} for {competitionTicketsOnly().tickets.length * price}€
          </Button>
        </Stack>
      </Grid>
      <Dialog
